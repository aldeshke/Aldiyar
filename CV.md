# Aldiyar
My Personal Academic &amp; Project Portfolio
> High School Student | Aspiring Fintech & Financial Economics

📍 Location: Astana, Kazakhstan
📧 Email: rahmankulaldiar33@gmail.com
✈️ Telegram: @teviw
🌐 GitHub: github.com/aldeshke 

---

## 👨‍🎓 About Me
A High School Senior with a specialization in Financial Economics and FinTech. I am passionate about market design, strategic decision-making, and algorithmic problem-solving. I plan to major in Corporate Finance at Chinese universities

---

## 🛠 Skills & Competencies

* Languages: Russian (Native), Kazakh (Native) English (C1 / IELTS 7.0), Chinese (Beginner)
* Programming Languages: Python
* Tools & Technologies: Git, GitHub, VS Code, Linux
* Soft Skills: Problem-solving, Teamwork, Project Management

---

## 🚀 Projects

### 📌 [Название проекта 1](ссылка-на-репозиторий)
* Описание: Краткое описание того, что делает проект (например, *Telegram-бот для расписания уроков*).
* Стек: Python, python-telegram-bot API.
* Результат: Ботом пользуются 50+ одноклассников.

### 📌 [Название проекта 2](ссылка-на-репозиторий)
* Описание: Веб-сайт или исследовательская работа.
* Стек: HTML, CSS, JavaScript.

---

## 🏆 Honors & Awards
* CMS (10 years of training)
* American Olympiad - 1st place
* SASMO - 2nd place
* Future participant of "Generation" Management Program

---

## 📜 Certifications
* 
* 
